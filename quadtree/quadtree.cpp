...
class qNode {
	public:
		...
		qNode(qNode *p, double x1, double y1, double x2, double y2): parent(p), x(x1), y(y1), x2(x2), y2(y2) {
			if (parent == NULL) {
				level = 0;
			} else {
		...
		void expand(bool set) {
			...
			for (int i = 0; i < 4; i++) {
				...
				children[i] = new qNode(this, newx, newy, newx + sideLenX, newy + sideLenY);
				for (int k = 0; k < 3; k++) {
